// ==UserScript==
// @name FS-Sign Full User Export XLSX-CSV
// @namespace http://tampermonkey.net/
// @version 4.0
// @description Полный сканер пользователей: все страницы, логирование, retry, export CSV/XLSX. Сохраняет прогресс в localStorage.
// @match https://sign.fsdocs.kz/users*
// @match https://sign.fsdocs.kz/users/*
// @match https://sign.fsdocs.kz/users
// @match https://signtest.fsdocs.kz/users*
// @match https://signtest.fsdocs.kz/users/*
// @match https://signtest.fsdocs.kz/users
// @grant none
...
(function () {
    'use strict';
    // ------------- Конфиг -------------
    const STORAGE_DATA = 'fsign_users_v3_data';
    const STORAGE_STATE = 'fsign_users_v3_state';
    const STORAGE_LOGS = 'fsign_users_v3_logs';
    ...
    <div class="row"><strong>FSign User Scanner</strong><span class="small">v3.0</span></div>
    ...
    function extractIdFromRow(row) {
        try {
            const a = row.querySelector('a[href*="/users/"], a[href*="/users/edit/"]');
            if (a && a.href) {
    ...
    function parseRowsFromDoc(doc, sourcePage) {
        ...
        for (const tr of rows) {
            ...
            const index = smartText(tds[0]) + (id ? ` (${id})` : '');
            const name = nameLink ? smartText(nameLink) : smartText(tds[1]);
            const email = smartText(tds[2]);
            const role = smartText(tds[3]);
            let companyText = smartText(tds[4]);
            let gln = '';
            ...
            parsed.push({
                id, index, name, email, role, gln, company
            });
        }
    ...
    function mergeParsed(parsed) {
        for (const it of parsed) {
            ...
            if (!exists && it.name && it.email) exists = data.find(d => d.name === it.name && d.email === it.email);
            if (!exists) data.push(it);
            else {
                for (const k of ['index','role','gln','company']) {
    ...
    function exportCSV(list, prefix) {
        const headers = ['ID','#','Имя','Email','Роль','GLN','Компания'];
        const rows = list.map(it=>[it.id,it.index,it.name,it.email,it.role,it.gln,it.company]);
    ...
    function exportXLSX(list, sheetName, prefix) {
        ...
        const headerOrder = ['ID','#','Имя','Email','Роль','GLN','Компания'];
    ...
    function exportAll() {
        ...
        exportCSV(data, 'Fsdocs_User_All');
        exportXLSX(data, 'AllUsers', 'Fsdocs_User_All');
    }
    // ------------- Main scanner -------------
    ...
    document.getElementById('fs-download-logs').addEventListener('click', () => {
        ...
        const a=document.createElement('a'); a.href=url; a.download = buildFilename('Fsign_User_export') + '.txt'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    ...
    pushLog('info','FSign Full User Export initialized');
    updateUI();
})();
